%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Education}

%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------

\cventry
  {M.D. in Innovation and Research in Informatics} % Degree
  {Universitat Polit\`ecnica de Catalunya (UPC) - FIB} % Institution
  {Barcelona, Spain} % Location
  {2018 - } % Date(s)
  {
    \begin{cvitems} % Description(s) bullet points
      \item {Specialization in \textit{Data Science}.}
    \end{cvitems}
  }

\cventry
  {B.S. in Computer Science} % Degree
  {Universitat Polit\`ecnica de Catalunya (UPC) - FIB} % Institution
  {Barcelona, Spain} % Location
  {2013 - 2017} % Date(s)
  {
    \begin{cvitems} % Description(s) bullet points
      \item {Coursed via \textit{Centre de Formaci\'o Interdisciplin\`aria Superior} (CFIS).}
      \item {Final project: ``Efficient Learning using Kernelized Associative Memories'' (A with honors).}
    \end{cvitems}
  }

  \cventry
    {B.S. in Telecommunications Engineering} % Degree
    {Universitat Polit\`ecnica de Catalunya (UPC) - ETSETB} % Institution
    {Barcelona, Spain} % Location
    {2012 - 2017} % Date(s)
    {
      \begin{cvitems} % Description(s) bullet points
        \item {Coursed via \textit{Centre de Formaci\'o Interdisciplin\`aria Superior} (CFIS).}
      \end{cvitems}
    }

%---------------------------------------------------------
\end{cventries}
